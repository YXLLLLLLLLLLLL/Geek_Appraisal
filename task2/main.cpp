...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] == 1)
        {
            arr_small[count2++] = arr_big[i];	//记录每个字母一共出现几次/*补充注释（说明作用）*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //如果两侧都为真则开始计算长度/*补充判断模块注释*/
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            sum = huffman[i][3] * length + sum;	//各个叶结点带权路径长度累加得到总长度WPL
        }
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断是否为最小结点/*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //如果不是则更改最小结点/*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //判断是否有父结点/*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    int first=0,last=29;     //char数组的头部和尾部

    while(father != -1)
    {
    	start++;
    	
        if(current == huffman[i][1]){/*自行填充代码*/		//判断当前结点的父结点左子树是否为当前结点
        	code[start] = '0';	//子结点是父结点的左子树，编码为0
        }
		else(current == huffman[i][2]);{/*自行填充代码*/
        	code[start] = '1';/*自行填充代码*/		//子结点是父结点的右子树，编码为1
		}
        current = huffman[i][0];/*自行填充代码*/		//往上朔源，更新当前结点
        father = huffman[i--][0];/*自行填充代码*/		//同理（当前结点更新后），更新当前结点的父亲结点
        first++ ;/*自行填充代码*/	//更新编码位置
    }
    code[start]='\0';   //编码结束符
