...
![](C:\Users\YXLLL\Desktop\Geek_Appraisal\task2\image.png)
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   答：有n-1个左叶子和右叶子以及最上面一个根节点，即2(n-1)+1=2n-1。

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

   答：小写：j + 32，同时编码：先对大写字母进行遍历，再对小写字母进行遍历。

3. 使用画图工具画出上述两个Huffman生成树的生成过程

   ![](C:\Users\YXLLL\Desktop\Geek_Appraisal\task2\画图1.png)

![](C:\Users\YXLLL\Desktop\Geek_Appraisal\task2\画图2.png)